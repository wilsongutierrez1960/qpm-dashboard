// =============================================================================
// QPM — Estimacion bayesiana del bloque externo AR(1) — MCMC CORTO -- sobre datos ROBUSTOS (2020Q2 tratado como NaN)
...
//
// mh_tune_jscale=0.33: en vez de fijar el tamano de paso a mano (que dio
// 63-64% de aceptacion, demasiado alto -> mala mezcla), Dynare busca
// automaticamente el jscale que acerque la tasa de aceptacion al objetivo
// (33%, el estandar de la literatura). mh_tune_guess=0.4 es el punto de
// partida de esa busqueda (el valor que ya probamos).
// -----------------------------------------------------------------------------
estimation(
    datafile = fred_data_dynare_robust,
    mode_compute = 0,
    mode_file = 'qpm_ar1_estimation_robust/Output/qpm_ar1_estimation_robust_mode',
    mh_replic = 20000,
    mh_nblocks = 2,
    mh_drop = 0.5,
    mh_tune_jscale = 0.33,
